...
class FileHandlerController extends Controller
{
    ...
    public function import(Request $request) 
    {
            ...
            if($key !== 0){
                ...
                //if($key == 2685 || $key == 2686 || $key == 2687 || $key == 9223 || $key == 5905 || $key == 16 || $key == 9227 || $key == 9239){
                    ...
                    if (array_key_exists($item[0], $values_be_replaced)) {
                        ...
                        //echo "wrong_rubric_name<br>";
                    }
                    
                    #if array has 9 elements
                    if (count($item) < 10 || empty($item[9])) {
                        ImportFileFixing::whenIncompleteCount($item);
                        $message->setMessage("empty_sub_rubric", $item[0], []);
                        //echo "empty_sub_rubric<br>";
                    }
                    #if an item name is empty - two first words from description are added
                    if(empty($item[4])){
                        ImportFileFixing::emptyNameBeFilled($item);
                        echo "key empty: ".$key."<br>";
                    }
                    echo "key: ".$key."<br>";
                    ...
                    $message->setMessage("manufacturer", $name_manufacturer, $this->manufacturerRepository->handler($item));
       ...
       /* echo "<pre>";
        ...
        echo "</pre>";*/
      $message->getMessage();
      return view('filehandler.result', [
        'message' => $message,
